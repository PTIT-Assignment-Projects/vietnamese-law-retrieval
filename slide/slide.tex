% !TEX program = xelatex
\documentclass[serif, aspectratio=169]{beamer}

\usepackage{fontspec}
\newfontfamily\myfont{Arial}
\usepackage{polyglossia}
\setmainlanguage{vietnamese}
\usepackage{hyperref}
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{multicol}
\usepackage{booktabs}
\usepackage{xcolor}

\usepackage{HKUSTstyle}
\usepackage{algorithm2e}

\definecolor{hkustblue}{RGB}{0, 56, 116}
\definecolor{hkustred}{RGB}{209, 51, 59}
\definecolor{codegreen}{RGB}{0, 128, 0}

\newcommand{\highlight}[1]{\textbf{\color{hkustblue}#1}}
\newcommand{\red}[1]{\textbf{\color{hkustred}#1}}
\newcommand{\green}[1]{\textbf{\color{codegreen}#1}}

\author{Vũ Công Tuấn Dương -- B22DCKH024}
\title{Tìm kiếm thông tin từ văn bản luật Việt Nam}
\institute{Khoa CNTT1 -- PTIT}
\date{\small \today}

\begin{document}

\begin{frame}
    \titlepage
\end{frame}

\begin{frame}{Mục lục}
    \tableofcontents
\end{frame}

% ========== SECTION 1: GIOI THIEU ==========
\section{Giới thiệu}
\begin{frame}{Mục tiêu đề tài}
\begin{itemize}
    \item Cài đặt và so sánh các phương pháp truy xuất thông tin:
    \begin{itemize}
        \item Mô hình Boolean
        \item Mô hình không gian vector (VSM) sử dụng TF-IDF
        \item Mô hình BM25
    \end{itemize}
    \item Sử dụng \textbf{Elasticsearch} làm baseline để so sánh
    \item Dữ liệu: Văn bản luật Việt Nam
    \item Xây dựng giao diện demo với Streamlit
\end{itemize}
\end{frame}

% ========== SECTION 2: CO SO LY THUYET ==========
\section{Cơ sở lý thuyết}

\subsection{Tiền xử lý}
\begin{frame}{Tại sao cần tiền xử lý tiếng Việt?}
\textbf{Đặc thù tiếng Việt:}
\begin{itemize}
    \item Từ ghép không có dấu cách: \textit{trách nhiệm}, \textit{phạm tội}
    \item Cùng một từ, nhiều cách viết: \textit{Việt Nam}, \textit{ViệtNam}
    \item Từ dừng phổ biến: \textit{là}, \textit{của}, \textit{và}...
\end{itemize}

\vspace{0.3cm}
\textbf{Các bước tiền xử lý:}
\begin{enumerate}
    \item Chuẩn hóa Unicode (NFC)
    \item Tách từ (tokenization)
    \item Loại bỏ từ dừng
    \item Chuyển về chữ thường
\end{enumerate}
\end{frame}

\begin{frame}{Tiền xử lý - Ví dụ}
\begin{columns}
\begin{column}{0.48\textwidth}
\textbf{Văn bản gốc:}\\
\textit{Người phạm tội sẽ bị truy cứu trách nhiệm hình sự}

\vspace{0.5cm}
\textit{Thuế GTGT của hàng hóa nhập khẩu}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Sau khi xử lý:}\\
\textit{người\_phạm\_tội truy\_cứu trách\_nhiệm hình\_sự}

\vspace{0.5cm}
\textit{thuế\_gtgt hàng\_hóa nhập\_khẩu}
\end{column}
\end{columns}

\vspace{0.5cm}
\green{Kết quả: Các từ ghép được giữ nguyên, loại bỏ từ dừng}
\end{frame}

\subsection{Chỉ mục ngược}
\begin{frame}{Chỉ mục ngược (Inverted Index)}
\textbf{Định nghĩa:} Cấu trúc dữ liệu ánh xạ từ \textbf{term} $\rightarrow$ danh sách tài liệu chứa term đó

\vspace{0.3cm}
\textbf{Hai thành phần chính:}
\begin{itemize}
    \item \textbf{Dictionary:} Tập hợp các từ duy nhất trong corpus
    \item \textbf{Postings List:} Danh sách (docID, term frequency) cho mỗi từ
\end{itemize}

\vspace{0.3cm}
\textbf{Ưu điểm:} Tra cứu $O(1)$ thay vì duyệt toàn bộ 222,981 tài liệu
\end{frame}

\begin{frame}{Chỉ mục ngược - Ví dụ}
\begin{table}
\centering
\small
\begin{tabular}{|l|l|}
\hline
\textbf{Term} & \textbf{Postings List} \\
\hline
thu\_gtgt & (Doc10, 5), (Doc25, 2), (Doc58, 3) \\
\hline
người\_phạm\_tội & (Doc1, 1), (Doc22, 3), (Doc33, 2) \\
\hline
hợp\_đồng & (Doc5, 4), (Doc12, 1), (Doc89, 2) \\
\hline
trách\_nhiệm & (Doc15, 2), (Doc30, 1), (Doc42, 3) \\
\hline
\end{tabular}
\end{table}

\vspace{0.3cm}
\green{Truy vấn "thu\_gtgt AND hợp\_đồng":} Lấy giao của Doc10, Doc25, Doc58 và Doc5, Doc12, Doc89
\end{frame}

\subsection{TF-IDF}
\begin{frame}{TF-IDF}
Đánh giá tầm quan trọng của từ dựa trên \cite{manning2008introduction}:
\begin{itemize}
    \item \textbf{TF (Term Frequency):} Tần suất xuất hiện của từ trong văn bản
    \item \textbf{IDF (Inverse Document Frequency):} Mức độ hiếm của từ trong tập tài liệu
\end{itemize}
\[
TF(t, d) = \frac{\text{Số lần xuất hiện của } t \text{ trong } d}{\text{Tổng số từ trong } d}
\]
\[
IDF(t, D) = \log \left( \frac{\text{Tổng số văn bản}}{\text{Số văn bản chứa } t} \right)
\]
\[
TF\text{-}IDF(t, d, D) = TF(t, d) \times IDF(t, D)
\]
\end{frame}

\begin{frame}{TF-IDF -- Thuật toán tính điểm}
\small
\textbf{Input:} Truy vấn $Q$, tài liệu $D$, chỉ mục ngược\\
\textbf{Output:} Điểm TF-IDF
\begin{enumerate}
    \item $score \leftarrow 0$
    \item \textbf{for} mỗi từ $t \in Q$ \textbf{do}
    \item \quad $tf \leftarrow$ số lần $t$ xuất hiện trong $D$
    \item \quad $idf \leftarrow \log\left(\frac{N + 1}{n_t + 1}\right) + 1$
    \item \quad $tf\_normalized \leftarrow 1 + \log(tf)$
    \item \quad $score \leftarrow score + tf\_normalized \times idf$
    \item \textbf{end for}
    \item \textbf{return} $score$
\end{enumerate}
\vspace{0.3cm}
\green{Tối ưu:} Chỉ tính TF-IDF cho các tài liệu ứng viên từ chỉ mục ngược
\end{frame}

\subsection{BM25}
\begin{frame}{BM25}
Biến thể nâng cao của TF-IDF \cite{manning2008introduction}, dựa trên mô hình xác suất:
\[
\text{score}(D, Q) = \sum_{i=1}^{n} IDF(q_i) \cdot \frac{f(q_i, D) \cdot (k_1 + 1)}{f(q_i, D) + k_1 \cdot (1 - b + b \cdot \frac{|D|}{\text{avgdl}})}
\]
\begin{itemize}
    \item $k_1$: Kiểm soát TF saturation ($1.2 - 2.0$)
    \item $b$: Chuẩn hóa độ dài tài liệu ($0.75$)
    \item Ưu điểm: Xử lý tốt độ bão hòa TF và chuẩn hóa độ dài
\end{itemize}
\end{frame}

\begin{frame}{BM25 -- Thuật toán tính điểm}
\footnotesize
\textbf{Input:} Truy vấn $Q$, chỉ mục ngược\\
\textbf{Output:} Top $N$ tài liệu có điểm cao nhất
\begin{enumerate}
    \item $k_1 \leftarrow 1.5$, $b \leftarrow 0.75$
    \item \textbf{Lấy candidates:} $C \leftarrow \bigcup_{q_i \in Q}$ các tài liệu chứa $q_i$
    \item \textbf{for} mỗi $D \in C$ \textbf{do}
    \item \quad $score \leftarrow 0$
    \item \quad \textbf{for} mỗi $q_i \in Q$ \textbf{do}
    \item \quad \quad $f \leftarrow$ số lần $q_i$ xuất hiện trong $D$
    \item \quad \quad $n_q \leftarrow$ số tài liệu chứa $q_i$
    \item \quad \quad $idf \leftarrow \ln\left(\frac{N - n_q + 0.5}{n_q + 0.5} + 1\right)$
    \item \quad \quad $num \leftarrow f \times (k_1 + 1)$
    \item \quad \quad $den \leftarrow f + k_1 \times (1 - b + b \times \frac{|D|}{avgdl})$
    \item \quad \quad $score \leftarrow score + idf \times \frac{num}{den}$
    \item \quad \textbf{end for}
    \item \quad Lưu $(D, score)$
    \item \textbf{end for}
    \item Sắp xếp theo score giảm dần, trả về top $N$
\end{enumerate}
\vspace{0.3cm}
\green{Độ phức tạp: $O(|C| \times |Q|)$ thay vì $O(N \times |Q|)$}
\end{frame}

\subsection{Các thước đo đánh giá}
\begin{frame}{Precision@K, Recall@K, F1@K}
\textbf{Precision@K:} Tỷ lệ tài liệu liên quan trong $k$ kết quả đầu tiên:
\[
\mathrm{P}@k = \frac{\#\{\text{tài liệu liên quan trong } k \text{ kết quả}\}}{k}
\]
\textbf{Recall@K:} Tỷ lệ tài liệu liên quan tìm được so với tổng số:
\[
\mathrm{R}@k = \frac{\#\{\text{tài liệu liên quan trong } k \text{ kết quả}\}}{\#\{\text{tổng tài liệu liên quan}\}}
\]
\textbf{F1@K:} Trung bình điều hòa:
\[
F1@k = 2 \cdot \frac{P@k \cdot R@k}{P@k + R@k}
\]
\end{frame}

\begin{frame}{Mean Reciprocal Rank (MRR)}
Tập trung vào vị trí của tài liệu liên quan \textbf{đầu tiên}:
\[
RR_i = \frac{1}{rank_i}, \quad MRR = \frac{1}{N} \sum_{i=1}^{N} \frac{1}{rank_i}
\]
\begin{itemize}
    \item Ưu tiên vị trí dẫn đầu (vị trí 1: 1 điểm, vị trí 2: 0.5 điểm)
    \item Phù hợp với hệ thống chỉ cần tìm thấy một kết quả đúng
\end{itemize}
\end{frame}

\begin{frame}{Mean Average Precision (MAP)}
Tính đến cả thứ hạng và độ chính xác:
\[
AP = \sum_{k=1}^{n} P(k) \cdot \Delta r(k)
\]
Trong đó:
\begin{itemize}
    \item $P(k) = \frac{\#\text{relevant trong top }k}{k}$: Precision\@k
    \item $\Delta r(k) = \frac{1}{|R|}$ nếu tài liệu tại $k$ là relevant, $=0$ nếu không
\end{itemize}
\[
MAP = \frac{1}{N} \sum_{i=1}^{N} AP_i
\]
\begin{itemize}
    \item Chỉ số phổ biến nhất trong đánh giá IR
    \item Đánh giá toàn diện hơn MRR
\end{itemize}
\end{frame}

\subsection{Elasticsearch}
\begin{frame}{Elasticsearch - Tổng quan}
\begin{itemize}
    \item Công cụ tìm kiếm phân tán, mã nguồn mở (Apache License)
    \item Dựa trên Apache Lucene
    \item CSDL NoSQL, lưu trữ tài liệu dạng JSON
    \item Sử dụng BM25 làm thước đo mặc định
\end{itemize}
\end{frame}


\begin{frame}{Elasticsearch - Truy vấn}
\begin{itemize}
    \item \textbf{Match Query:} Tìm kiếm toàn văn với phân tích token
    \item \textbf{Term Query:} Kết quả khớp chính xác
    \item \textbf{Bool Query:} Kết hợp must, should, must\_not
\end{itemize}
\end{frame}

% ========== SECTION 3: THUC NGHIEM ==========
\section{Thực nghiệm và đánh giá kết quả}
\begin{frame}{Tập dữ liệu}
\begin{itemize}
    \item Nguồn: Bộ dữ liệu văn bản pháp luật Việt Nam trên HuggingFace \cite{dataset}
    \item \textbf{Corpus:} 222,981 tài liệu (các điều, khoản luật)
    \item \textbf{Train:} 9,541 câu hỏi
    \item \textbf{Test:} 3,184 câu hỏi
    \item Sử dụng 2,000 mẫu đầu tiên để đánh giá (giới hạn phần cứng, thời gian chạy)
\end{itemize}
\end{frame}

\begin{frame}{Cấu trúc tập dữ liệu}
\begin{table}
\centering
\small
\begin{tabular}{|l|l|l|}
\hline
Cột & Kiểu & Mô tả \\ \hline
\texttt{qid} & int & ID câu hỏi \\ \hline
\texttt{question} & string & Câu hỏi \\ \hline
\texttt{cid} & list & ID văn bản liên quan \\ \hline
\texttt{context\_list} & list & Nội dung liên quan \\ \hline
\end{tabular}
\end{table}
\end{frame}

\begin{frame}{Tiền xử lý và xây dựng chỉ mục}
\begin{itemize}
    \item Sử dụng \texttt{underthesea} \cite{underthesea} để chuẩn hóa và tách từ
    \item Token có 2 từ trở lên được nối bằng dấu "\_"
    \item Loại bỏ từ dừng bằng danh sách từ Github \cite{vietnamese_stopwords}
\end{itemize}
\vspace{0.3cm}
\textbf{Elasticsearch - 2 cách tiếp cận:}
\begin{itemize}
    \item \textbf{Chỉ mục mặc định:} Analyzer chuẩn của ES (tiếng Anh)
    \item \textbf{Chỉ mục đã xử lý:} Tách từ tiếng Việt bằng \texttt{underthesea}
\end{itemize}
\vspace{0.3cm}
\textbf{Mapping:} \texttt{content (text)} cho full-text, \texttt{content.keyword} cho exact match
\end{frame}

\begin{frame}{Các phương pháp truy xuất}
5 phương pháp được cài đặt và đánh giá:
\begin{enumerate}
    \item \textbf{Boolean}: Truy vấn với toán tử AND, OR, NOT
    \item \textbf{VSM}: Mô hình không gian vector với TF-IDF
    \item \textbf{BM25}: Mô hình xác suất với TF saturation
    \item \textbf{Elasticsearch (Thường)}: Analyzer mặc định
    \item \textbf{Elasticsearch (Đã xử lý)}: Tách từ trước bằng underthesea
\end{enumerate}
\end{frame}

\begin{frame}{Kết quả tổng quan}
\begin{table}
\centering
\small
\begin{tabular}{lccccccc}
\hline
Phương pháp & P@1 & P@5 & P@10 & R@10 & F1@10 & MRR & MAP \\
\hline
\textbf{ES (Xử lý)} & \textbf{0.4132} & \textbf{0.1349} & \textbf{0.0771} & \textbf{0.7708} & \textbf{0.1401} & \textbf{0.5284} & \textbf{0.5284} \\
BM25 & 0.4120 & 0.1358 & 0.0770 & 0.7695 & 0.1399 & 0.5269 & 0.5269 \\
ES (Thường) & 0.4012 & 0.1339 & 0.0737 & 0.7372 & 0.1340 & 0.5136 & 0.5136 \\
VSM & 0.2195 & 0.0963 & 0.0627 & 0.6270 & 0.1140 & 0.3353 & 0.3353 \\
Boolean & 0.1305 & 0.0541 & 0.0306 & 0.3060 & 0.0556 & 0.1900 & 0.1900 \\
\hline
\end{tabular}
\end{table}
\end{frame}

\begin{frame}{Kết quả chi tiết - BM25}
\begin{table}
\centering
\begin{tabular}{cccc}
\hline
K & Precision@K & Recall@K & F1@K \\
\hline
1  & 0.4120 & 0.4120 & 0.4120 \\
2  & 0.2685 & 0.5370 & 0.3580 \\
3  & 0.2033 & 0.6100 & 0.3050 \\
5  & 0.1358 & 0.6790 & 0.2263 \\
10 & 0.0769 & 0.7695 & 0.1399 \\
\hline
\end{tabular}
\end{table}
\end{frame}

\begin{frame}{Kết quả chi tiết - ES (Đã xử lý)}
\begin{table}
\centering
\begin{tabular}{cccc}
\hline
K & Precision@K & Recall@K & F1@K \\
\hline
1  & 0.4132 & 0.4132 & 0.4132 \\
2  & 0.2716 & 0.5432 & 0.3621 \\
3  & 0.2043 & 0.6128 & 0.3064 \\
5  & 0.1349 & 0.6744 & 0.2248 \\
10 & 0.0771 & 0.7708 & 0.1401 \\
\hline
\end{tabular}
\end{table}
\end{frame}

\begin{frame}{Nhận xét}
\begin{itemize}
    \item \textbf{BM25 vs ES (xử lý):} Hiệu suất gần tương đương (MRR chênh 0.0015)
    \item \textbf{ES xử lý vs ES thường:} Tiền xử lý tách từ tiếng Việt giúp cải thiện kết quả
    \item \textbf{BM25 vs VSM:} BM25 vượt trội nhờ độ bão hòa TF và chuẩn hóa độ dài
    \item \textbf{Boolean:} Hiệu suất thấp nhất, không có xếp hạng
    \item \textbf{Xu hướng:} P@K giảm, R@K tăng khi K tăng
\end{itemize}
\end{frame}


\begin{frame}{Giao diện demo}
\begin{figure}
        \centering
        \includegraphics[height=0.8\textheight]{pic/demo-ui.png}
        \label{fig:demo-ui}
\end{figure}
\end{frame}

\begin{frame}{Kết luận}
\begin{itemize}
    \item Đã cài đặt và so sánh thành công 5 phương pháp truy xuất
    \item \textbf{BM25} và \textbf{ES (đã xử lý)} cho kết quả tốt nhất
    \item Tiền xử lý văn bản (tách từ tiếng Việt) quan trọng
    \item Xây dựng giao diện demo hoàn chỉnh với Streamlit \cite{streamlit}
\end{itemize}
\end{frame}

\begin{frame}
    \begin{center}
        Cảm ơn thầy đã lắng nghe!
    \end{center}
\end{frame}

\section{Tài liệu tham khảo}
\begin{frame}[t, allowframebreaks]
    \fontsize{9pt}{10pt}\selectfont
    \textbf{Tài liệu tham khảo}
    \bibliographystyle{plain}
    \bibliography{sample}
\end{frame}

\end{document}
